    ...
    void OnTriggerEnter(Collider col)
    {
        if (TriggerDamageAllowed && !playerWeapon)
        {
            if (col.gameObject.CompareTag("Player"))
            {
                ...
                {
                    WeaponTypeSwitch.SetValue(gameObject);
                    alreadyHitObjects.Add(col.gameObject);
                    WeaponImpact.Post(transform.parent.gameObject);
                    GameManager.DamageObject(col.gameObject, attack);
                }

                //TODO: Make check to make sure that damage is not spammed on the player.
                //this.enabled = false;
            }
        }
    }
        ...
        if (playerWeapon)
        {
            if (equipped && col.gameObject.tag != "Player")
            {
                ...
                if (!alreadyHitObjects.Contains(col.gameObject))
                {
                    ...
                    if (sm != null)
                    {
                        sm.material.SetValue(transform.parent.gameObject);
                        sm.material.SetValue(sm.gameObject);
                    }
                    WeaponTypeSwitch.SetValue(transform.parent.gameObject);

                    alreadyHitObjects.Add(col.gameObject);
                    WeaponImpact.Post(transform.parent.gameObject);
                    GameManager.DamageObject(col.gameObject, attack);

                    //Debug.Log("Hit " + col.gameObject + " with "+attack.swingType);
